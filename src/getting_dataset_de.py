...
num_employees = 630
start_year = 2010  # Das Jahr, in dem die Simulation startet
start_month = 1  # Der Monat, in dem die Simulation startet
...
def generate_employee_data(position, current_year, current_month):
    """
    Generiert Mitarbeiterdaten, wobei das Einstellungsdatum immer zum aktuellen Jahr und Monat passt.

    Parameter:
    - position: Position des Mitarbeiters.
    - current_year: Das aktuelle Simulationsjahr (int).
    - current_month: Der aktuelle Simulationsmonat (int).

    Rückgabewerte:
    - Alter: Alter des Mitarbeiters.
    - Geburtsdatum: Berechnetes Geburtsdatum basierend auf dem Alter.
    - Einstellungsdatum: Fixiert auf das aktuelle Jahr und den aktuellen Monat.
    """
    ...
    # Altersverteilung basierend auf Position bestimmen
    ...
    birth_month = random.randint(1, 12)  # Zufälliger Monat
    birth_day = random.randint(1, 28)  # Flexibler Tag (ohne komplexen Monat-Tag-Abgleich)
    birth_date = date(birth_year, birth_month, birth_day)

    # Mindestdatum für das Einstellungsdatum berechnen
    min_valid_hiring_date = birth_date + timedelta(days=25 * 365)  # Mindestalter 25 Jahre

    # Einstellungsdatum: Immer der 01. des Monats
    hiring_date = date(current_year, current_month, 1)


    # Stellen Sie sicher, dass das Einstellungsdatum das Mindestdatum erfüllt
    if hiring_date < min_valid_hiring_date:
        # Wenn das aktuelle Einstellungsdatum ungültig ist, wird es korrigiert
...
def generate_valid_hiring_date(birth_date, year, month, min_age=25):
    """
    Generiert ein gültiges Einstellungsdatum, das zu einem angegebenen Jahr und Monat passt,
    und überprüft eine Altersgrenze.

    :param birth_date: Geburtsdatum des Mitarbeiters (datetime.date)
    :param year: Jahr, in dem das Einstellungsdatum generiert werden soll (int)
    :param month: Monat, in dem das Einstellungsdatum generiert werden soll (int)
    :param min_age: Mindestalter, das der Mitarbeiter erreicht haben muss (Standard: 25 Jahre)
    :return: Ein gültiges datetime.date-Objekt, das das generierte Einstellungsdatum enthält.
    """


    # Mindestjahr berechnen (Geburtsjahr + Mindestalter)
    min_hiring_year = birth_date.year + min_age

    # Sicherstellen, dass das Einstellungsjahr >= min_hiring_year liegt
    if year < min_hiring_year:
        raise ValueError(f"Einstellungsdatum nicht möglich: Jahr {year} ist vor dem Mindestjahr {min_hiring_year}.")

    # Sicherstellen, dass das Einstellungsmonat gültig ist für den aktuellen Kontext
    if month not in range(1, 13):
        raise ValueError(f"Ungültiger Monat: {month}. Monat muss zwischen 1 und 12 liegen.")

    # Generiere den genauen Tag (zwischen 1 und Tagen des Monats)
    if month == 2:  # Spezieller Fall für Februar
        day = random.randint(1, 28)  # Kein Schaltjahr-Handling nötig, da Simulation
    elif month in [4, 6, 9, 11]:  # Monate mit 30 Tagen
        day = random.randint(1, 30)
    else:  # Alle anderen Monate mit 31 Tagen
        day = random.randint(1, 31)

    # Erstelle das valide Einstellungsdatum
    hiring_date = date(year, month, day)

    # Validierung des Alters: Prüfen, ob Geburtsdatum + Mindestalter vor dem Einstellungsdatum liegt
    min_valid_date = date(min_hiring_year, birth_date.month, birth_date.day)
    if hiring_date < min_valid_date:
        raise ValueError(f"Einstellungsdatum {hiring_date} ist vor Erreichen des Mindestalters {min_valid_date}.")
...
def generate_illness_days(satisfaction, age, work_model, leadership_stress=False,
    ...
    """
    Generiert Krankheitstage und berücksichtigt eine Begrenzung in den ersten drei Monaten.

    :param satisfaction: Zufriedenheitslevel des Mitarbeiters
    :param age: Alter des Mitarbeiters
    :param work_model: Arbeitsmodell (z. B. Vollzeit/Teilzeit)
    :param leadership_stress: Ob die Position Führungsverantwortung mitbringt
    :param long_term_illness_prob: Wahrscheinlichkeit für langwierige Erkrankungen
    :param hire_date: Einstellungsdatum des Mitarbeiters
    :param current_date: Aktuelles Datum
    :return: Anzahl der Krankheitstage
    """
    # Falls Hire Date und Current Date gesetzt sind, prüfe auf drei Monate Begrenzung
    if hire_date and current_date:
        hire_duration = (current_date - hire_date).days
        if hire_duration <= 90:  # Erste drei Monate (90 Tage)
            illness_days = max(0, int(np.random.normal(loc=1, scale=1)))  # Kleine Varianz innerhalb Begrenzung
            return min(3, illness_days)  # Maximale Krankheitstage in den ersten 3 Monaten: 3

    # Normale Berechnung der Krankheitstage außerhalb der ersten drei Monate
...
for i in range(num_employees):
    ...
    # Mitarbeiterdaten (Alter, Geburtsdatum, Einstellungsdatum)
    ...
    for month in months:
        ...
        for eintrag in list(nachfolgeplanung):  # Kopie der Nachfolgeplanung, um Änderungen zu ermöglichen
            if eintrag["Jahr"] == year and eintrag["Monat"] == month:
                ...
                # Position aus der Nachfolgeplanung
                position = eintrag["Position"]

                # Generiere Mitarbeiterdaten mit aktuellem Jahr und Monat
                ...
                illness_days_calculated = generate_illness_days(
                    satisfaction=random.randint(1, 10),  # Zufällige Zufriedenheitsbewertung
                    age=age,
                    work_model=random.choice(work_models),  # Zufälliges Arbeitsmodell
                    leadership_stress=(position in leadership_positions),  # Führungsverantwortung prüfen
                    hire_date=employee_data["Einstellungsdatum"],  # Einstellungsdatum des neuen Mitarbeiters
                    current_date=date.today()  # Heutiges Datum
                )
        ...
        if abs(abweichung) > 33:
            ...
            for _ in range(zu_hinzufuegende_mitarbeiter):
                ...
                employee_data = generate_employee_data(position, year, month)  # Korrekte Argumente
...
df_updated.to_csv("../data/HR_Testdatensatz.csv", index=False, date_format="%Y-%m-%d")
