...
def update_status_sums(selected_year):
    ...
    new_hires_df = df[
        (df["Einstellungsdatum"].notna()) &
        (df["Einstellungsdatum"].dt.year == selected_year)
        ]
    num_new_hires = new_hires_df["Mitarbeiter_ID"].nunique()

    # FILTER: Ausgeschiedene (entspricht Filterung in Code 2)
    exited_df = df[
        (df["Status"] == "Ausgeschieden") &
        (df["Austrittsdatum"].notna()) &
        (df["Austrittsdatum"].dt.year == selected_year)
        ]
    num_exits = exited_df["Mitarbeiter_ID"].nunique()
    ...
    num_retired = retired_df["Mitarbeiter_ID"].nunique()
