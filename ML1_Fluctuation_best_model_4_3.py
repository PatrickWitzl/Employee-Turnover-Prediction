...
def reduce_dimensions(X_train, X_test, X_full, n_components=0.95):
    """
...
def get_critical_employees_all_models(models, X_transformed, df, scaler_file="Models/scaler.pkl",
    ...
    for model_name, model in models.items():
        ...
        try:
            # ** Spezialbehandlung für PCA je nach Modell **
            X_model_transformed = X_transformed

            if isinstance(model, LogisticRegression):
                # PCA nur anwenden, wenn das Modell LogisticRegression ist
                if pca is not None:
                    try:
                    ...
                    print(f"Kein PCA-Objekt übergeben. Logistische Regression '{model_name}' ohne PCA.")
            elif isinstance(model, lgb.LGBMClassifier):
                # Für LightGBM geladene Feature-Namen anwenden
                ...
                print(
                    f"PCA und Feature-Namen-Anpassung werden für '{model_name}' nicht angewendet (nicht erforderlich).")

            # ** Vorhersagen für jedes Modell berechnen **
            try:
                if hasattr(model, "predict_proba"):
                ...
                print(f"Fehler bei der Vorhersage für Modell '{model_name}': {e}")
                results[model_name] = (None, None)
                continue
...
def compare_model_top_employees(file_paths, output_file="Outputs/Vergleich_Top_15.csv"):
    """
    Lädt die Top-15-Mitarbeiter-Dateien für verschiedene Modelle, vergleicht Namen, zählt,
    wie oft ein Name in den verschiedenen Modellen vorkommt, gibt Fluktuationswahrscheinlichkeiten
    je Modell in Prozent an und erstellt eine kompakte Zusammenfassung.
    ...
    Returns:
        pd.DataFrame: Kompakter DataFrame mit Namen, Häufigkeiten, Fluktuationswahrscheinlichkeiten (in Prozent) und Übereinstimmung.
    """
    # Dictionary zum Speichern der Daten pro Modell
    model_data = {}

    print("Lade Top-15-Mitarbeiter-Dateien...")
    # Dateien aus allen Modellen laden
    for model, file_path in file_paths.items():
        try:
            print(f"Lade Datei für Modell '{model}' aus {file_path}...")
            df = pd.read_csv(file_path)  # CSV-Datei laden
    ...
    # Kombiniere alle geladenen Daten in einen DataFrame
    ...
    # Zähle, wie oft jeder Name vorkommt (über alle Modelle hinweg)
    print("\nZähle, wie oft jeder Name in allen Modellen vorkommt...")
    combined_df["Häufigkeit_in_Modellen"] = combined_df.notna().sum(
        axis=1) - 1  # Ignoriere 'Name'-Spalte bei der Zählung

    # Überprüfen auf gleiche Namen in mehreren Modellen
    combined_df["Übereinstimmung"] = combined_df["Häufigkeit_in_Modellen"] > 1

    # Sortiere die Daten alphabetisch nach Name
